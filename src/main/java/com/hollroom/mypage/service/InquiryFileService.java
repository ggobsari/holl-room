...
public class InquiryFileService {
    ...
    public List<InquiryAttatchDTO> uploadFiles(List<MultipartFile> multipartFiles) throws IOException {
        List<InquiryAttatchDTO> fileDTOList = new ArrayList<>();
        for (MultipartFile multipartFile:multipartFiles){
            if(!multipartFile.isEmpty()){
                String storeFileName = uploadFile(multipartFile);
                //원본, 저장되는 파일 이름만 dto로 전달
                fileDTOList.add(new InquiryAttatchDTO(null, TabType.MYPAGE ,multipartFile.getOriginalFilename(),storeFileName));
            }
        }
        return fileDTOList;
    }
